# Konado: Marco de Motor de Novela Visual

[English](README.md) | [简体中文](README.zh-CN.md) | [繁體中文](README.zh-TW.md) | [日本語](README.ja.md) | [한국어](README.ko.md) | Español


<div align="center" style="border-radius: 12px; overflow: hidden">
<img src="./assets/kona/kona_lantern.png" alt="Konado Mascot" width="50%" max-width="500px">
</div>

<br>

<div align="center">
  <img src="https://img.shields.io/badge/License-BSD_3--Clause-orange.svg?style=flat-square&logo=opensourceinitiative&logoSize=14" alt="License" height="20">
  <img src="https://img.shields.io/github/last-commit/godothub/konado.svg?style=flat-square&logo=github&logoSize=14" alt="Last Commit" height="20">
  <img src="https://img.shields.io/github/release/godothub/konado.svg?style=flat-square&logo=github&logoSize=14" alt="Latest Release" height="20">
  <img src="https://img.shields.io/github/forks/godothub/konado?style=flat-square&logo=github&color=blueviolet&logoSize=14" alt="Forks" height="20">
  
  <img src="https://img.shields.io/github/stars/godothub/konado?style=flat-square&logo=github&color=ffcb2b&logoSize=14" alt="Stars" height="20">
  <img src="https://atomgit.com/godothub/konado/star/badge.svg?style=flat-square&logoSize=14" alt="AtomGit GStar" height="20">
  <img src="https://atomgit.com/godothub/konado/star/2025top.svg?style=flat-square&logoSize=14" alt="AtomGit GStar 2025" height="20">
</div>

<br>



## Descripción General

Konado es un marco ligero de motor de novela visual que proporciona un amplio conjunto de plantillas de diálogo y un robusto administrador de diálogos. Diseñado para flexibilidad, permite a los desarrolladores crear rápidamente juegos impulsados por diálogos o integrar sin problemas sistemas de diálogo en proyectos de juego existentes.

## Inicio Rápido

Ponte en marcha con Konado en minutos siguiendo nuestra [Guía de Inicio Rápido](https://godothub.com/oss/konado/tutorial/install.html).

## Documentación

Para guías completas, referencias de API y tutoriales avanzados, visita nuestro sitio web oficial del proyecto: https://godothub.com/oss/konado.

## Comunidad

Únete a nuestra comunidad para conectarte con otros usuarios de Konado, compartir ideas y obtener apoyo. Puedes participar a través de:

- Grupo de Discusión del Canal QQ: [Únete Ahora](https://pd.qq.com/g/godot)
- Canal del Servidor Discord: [Únete Ahora](https://discord.com/channels/1378639076747513938/1425084240550166592)

<a href="https://godothub.com">
<img src="https://godothub.com/licon/godothub.png" alt="godothub" style="width: 65%;" />
</a>

## Contribuir

¿Interesado en contribuir a Konado? Consulta nuestro [CONTRIBUTING.md](./CONTRIBUTING.md) para obtener guías detalladas sobre cómo participar.

## Código de Conducta

Este proyecto se adhiere a nuestro [Código de Conducta](./CODE_OF_CONDUCT.md). Al participar, se espera que mantengas este código.

## Contribuidores

Una lista completa de contribuidores se puede encontrar en [AUTHORS.md](./AUTHORS.md).

## Licencia de Código Abierto

Konado se distribuye bajo la Licencia BSD 3-Clause. Consulta el archivo [LICENSE](./LICENSE) para ver los términos y condiciones completos.